...
function FarmHouseCard({ farmHouse }) {
  const navigate = useNavigate();
  const [isShortlisted, setIsShortlisted] = useState(false);

  const handleCardClick = () => {
    navigate(`/venue-hall/farmhouse/${farmHouse._id}`);
  };
  ...
  const handleShare = (e) => {
    ...
    if (navigator.share) {
      navigator.share({
        ...
        text: `Check out ${farmHouse.name} in ${farmHouse.location.city}`,
        url: window.location.href + `/venue-hall/farmhouse/${farmHouse._id}`,
  ...
  return (
    ...
    >
      ...
      {/* Content Section */}
      <div className="p-6">
        <h3 className="text-xl font-bold text-gray-800 mb-2">{farmHouse.name}</h3>
        ...
        <div className="flex items-center text-gray-600 mb-3">
          ...
          <span className="text-sm">{farmHouse.location.area}, {farmHouse.location.city}</span>
        </div>
        ...
        <div className="flex items-center text-gray-600 mb-3">
          ...
          <span className="text-sm">Capacity: {farmHouse.capacity.min} - {farmHouse.capacity.max} guests</span>
        </div>
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            {farmHouse.price.perPlate && (
              <p className="text-sm text-gray-500">
                ₹{farmHouse.price.perPlate}/plate
              </p>
            )}
          </div>
          <div className="flex items-center">
            ...
            <span className="ml-1 text-sm font-semibold text-gray-700">
              {farmHouse.ratings.average} ({farmHouse.ratings.count})
            </span>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          ...
          {farmHouse.amenities.length > 3 && (
            <span className="text-xs bg-green-100 text-green-600 px-2 py-1 rounded-full font-semibold">
              +{farmHouse.amenities.length - 3} more
            </span>
          )}
        </div>
