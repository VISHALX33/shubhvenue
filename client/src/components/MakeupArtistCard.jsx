...
import { FaMapMarkerAlt, FaStar, FaPaintBrush, FaBriefcase, FaHeart, FaShare } from 'react-icons/fa';

const MakeupArtistCard = ({ artist }) => {
  const navigate = useNavigate();

  const handleCardClick = () => {
    navigate(`/event-services/makeup-artist/${artist._id}`);
  };
    ...
    >
      ...
      <div className="relative h-56 overflow-hidden group">
        ...
        <div className="absolute top-4 right-4 bg-white px-3 py-1 rounded-full shadow-md">
          <span className="text-purple-600 font-bold">₹{(minPrice / 1000).toFixed(0)}k+</span>
        </div>
      ...
      <div className="p-5">
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          ...
          <div className="flex items-center">
            <FaPaintBrush className="text-purple-500 mr-2" />
            <div>
        ...
        <div className="mb-4">
          <div className="flex flex-wrap gap-2">
            {artist.makeupStyles.slice(0, 3).map((style, index) => (
              <span 
                key={index}
            ...
            {artist.makeupStyles.length > 3 && (
              <span className="bg-gray-100 text-gray-600 px-3 py-1 rounded-full text-xs font-medium">
                +{artist.makeupStyles.length - 3} more
              </span>
            )}
          </div>
        </div>
        ...
        <button
          ...
          className="w-full mt-4 bg-gradient-to-r from-purple-600 to-fuchsia-600 text-white py-3 rounded-lg font-semibold hover:from-purple-700 hover:to-fuchsia-700 transition-all duration-300 shadow-md hover:shadow-lg"
        >
          Book Now
        </button>
      </div>
