...
function ResortCard({ resort }) {
  ...
  const handleCardClick = () => {
    navigate(`/venue-hall/resort/${resort._id}`);
  };
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: resort.name,
          text: `Check out ${resort.name} - ${resort.type}`,
    ...
    >
      ...
      <div className="relative h-64 overflow-hidden">
        ...
        <div className="absolute top-4 left-4">
          <span className="bg-teal-600 text-white px-3 py-1 rounded-full text-sm font-semibold shadow-lg">
            {resort.type}
          </span>
        </div>
      ...
      <div className="p-6">
        ...
        <div className="flex items-center text-gray-600 mb-3">
          ...
          <span className="text-sm">{resort.location.area}, {resort.location.city}</span>
        </div>
        ...
        <div className="flex items-center justify-between mb-3 text-sm text-gray-600">
          ...
          <div className="flex items-center">
            ...
            <span>{resort.capacity.min} - {resort.capacity.max}</span>
          </div>
        </div>
        ...
        <div className="flex items-center mb-4">
          ...
          <span className="ml-2 text-gray-700 font-semibold">{resort.ratings.average}</span>
          <span className="ml-1 text-gray-500 text-sm">({resort.ratings.count} reviews)</span>
        </div>
