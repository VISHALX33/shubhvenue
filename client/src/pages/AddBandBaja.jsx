...
function AddBandBaja() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:5000/api/band-bajas', formData, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (response.data.success) {
        alert('Band Baja listing created successfully!');
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-lg p-8">
        <h1 className="text-3xl font-bold text-gray-800 mb-6">Add Band Baja Service</h1>
