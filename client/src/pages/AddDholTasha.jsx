...
function AddDholTasha() {
  ...
  const [formData, setFormData] = useState({
    ...
    groupMembers: '', dholPlayers: '', tashaPlayers: '',
    instruments: [], costumes: { included: true, types: [] },
    performances: [], choreography: false, formations: [],
    price: { perEvent: '', perHour: '' },
    duration: { minimum: '', maximum: '' },
    lightingIncluded: false, soundSystemIncluded: false,
  ...
  const dholTashaTypes = ['Traditional Dhol Tasha', 'Pathak Style', 'Modern Fusion', 'Competition Group', 'Festival Specialist', 'Budget Group'];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        alert('Dhol Tasha listing created successfully!');
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-lg p-8">
        <h1 className="text-3xl font-bold text-gray-800 mb-6">Add Dhol Tasha Service</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-4">
            ...
            <div>
              ...
              <select name="type" value={formData.type} onChange={handleChange} required className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                ...
                {dholTashaTypes.map(type => <option key={type} value={type}>{type}</option>)}
              </select>
            </div>
            <div className="grid grid-cols-3 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Total Members (10-100) *</label>
                <input type="number" name="groupMembers" value={formData.groupMembers} onChange={handleChange} required min="10" max="100" className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Dhol Players *</label>
                <input type="number" name="dholPlayers" value={formData.dholPlayers} onChange={handleChange} required min="0" className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Tasha Players *</label>
                <input type="number" name="tashaPlayers" value={formData.tashaPlayers} onChange={handleChange} required min="0" className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" />
              </div>
            </div>
            <div>
          ...
          {/* Instruments, Performances, Features, Formations - Similar pattern */}
          <div className="space-y-4">
            <h2 className="text-xl font-semibold text-gray-700">Instruments</h2>
            <div className="flex gap-2">
              <input type="text" value={instrumentInput} onChange={(e) => setInstrumentInput(e.target.value)} className="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" placeholder="e.g., Dhol, Tasha, Cymbals" />
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              <input type="text" value={performanceInput} onChange={(e) => setPerformanceInput(e.target.value)} className="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" placeholder="e.g., Wedding Procession, Festival" />
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              <input type="text" value={featureInput} onChange={(e) => setFeatureInput(e.target.value)} className="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" placeholder="e.g., Synchronized Performance, LED Dhols" />
          ...
          <div className="space-y-4">
            <h2 className="text-xl font-semibold text-gray-700">Choreography & Formations</h2>
            <label className="flex items-center gap-2 mb-4">
              <input type="checkbox" name="choreography" checked={formData.choreography} onChange={handleChange} className="w-4 h-4" />
              <span className="text-sm font-medium text-gray-700">Choreography Available</span>
            </label>
            {formData.choreography && (
              <>
                <div className="flex gap-2">
                  <input type="text" value={formationInput} onChange={(e) => setFormationInput(e.target.value)} className="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" placeholder="e.g., Circle, Line, Triangle" />
                  <button type="button" onClick={() => addItem('formations', formationInput, setFormationInput)} className="px-6 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600">Add</button>
                </div>
                <div className="flex flex-wrap gap-2">
                  {formData.formations.map((item, idx) => (
                    <span key={idx} className="bg-orange-100 text-orange-800 px-3 py-1 rounded-full flex items-center gap-2">
                      {item}
                      <button type="button" onClick={() => removeItem('formations', idx)} className="text-orange-600 hover:text-orange-800">×</button>
                    </span>
                  ))}
                </div>
              </>
          ...
          <div className="space-y-4">
            ...
            {formData.costumes.included && (
              <>
                <div className="flex gap-2">
                  <input type="text" value={costumeInput} onChange={(e) => setCostumeInput(e.target.value)} className="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" placeholder="e.g., Traditional, Pathak Style" />
          ...
          <div>
            ...
            <textarea name="about" value={formData.about} onChange={handleChange} required rows="4" className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" placeholder="Describe your Dhol Tasha group..." />
          </div>
