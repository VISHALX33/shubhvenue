...
const BandBajaDetails = () => {
  const { id } = useParams();
  const navigate = useNavigate();
  const { user } = useAuth();
  const [bandBaja, setBandBaja] = useState(null);
  const [loading, setLoading] = useState(true);
  const [activeTab, setActiveTab] = useState('about');
  const [selectedImage, setSelectedImage] = useState(0);

  useEffect(() => {
    fetchBandBajaDetails();
  }, [id]);

  const fetchBandBajaDetails = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`http://localhost:5000/api/band-baja/${id}`);
      setBandBaja(response.data.data);
    } catch (error) {
      console.error('Error fetching band baja details:', error);
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-gray-800 mb-4">Band Baja not found</h2>
          <button
            onClick={() => navigate('/event-services/band-baja')}
  ...
  const allImages = [bandBaja.mainImage, ...bandBaja.images];
            ...
            <div className="grid grid-cols-3 lg:grid-cols-1 gap-4">
              {allImages.map((image, index) => (
                <img
                  key={index}
                  src={image}
                  alt={`${bandBaja.name} ${index + 1}`}
                  className={`w-full h-32 object-cover rounded-lg cursor-pointer transition-all ${
                    selectedImage === index ? 'ring-4 ring-purple-600' : 'hover:opacity-75'
                  }`}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            ...
            <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
              <h1 className="text-3xl font-bold text-gray-800 mb-2">{bandBaja.name}</h1>
              <div className="flex items-center space-x-4 mb-4">
                ...
                <div className="flex items-center text-gray-600">
                  ...
                  <span>{bandBaja.location.area}, {bandBaja.location.city}, {bandBaja.location.state}</span>
                </div>
                <div className="flex items-center">
                  <div className="flex mr-2">
                    {[...Array(5)].map((_, i) => (
                      <svg
                        key={i}
                        className={`w-5 h-5 ${i < Math.floor(bandBaja.ratings.average) ? 'text-yellow-500' : 'text-gray-300'}`}
                        fill="currentColor"
                        viewBox="0 0 20 20"
                      >
                  ...
                  <span className="text-gray-600">({bandBaja.ratings.count} reviews)</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-lg shadow-lg mb-6">
              ...
              <div className="p-6">
                ...
                {activeTab === 'about' && (
                  <div>
                    <h2 className="text-2xl font-bold text-gray-800 mb-4">About {bandBaja.name}</h2>
                    <p className="text-gray-700 mb-6 leading-relaxed">{bandBaja.about}</p>

                    <div className="grid grid-cols-2 gap-4 mb-6">
                      <div className="bg-purple-50 p-4 rounded-lg">
                        <div className="text-sm text-gray-600 mb-1">Band Members</div>
                        <div className="text-2xl font-bold text-purple-600">{bandBaja.bandMembers}</div>
                      </div>
                      <div className="bg-purple-50 p-4 rounded-lg">
                        <div className="text-sm text-gray-600 mb-1">Dhol Players</div>
                        <div className="text-2xl font-bold text-purple-600">{bandBaja.dholPlayers}</div>
                      </div>
                      <div className="bg-purple-50 p-4 rounded-lg">
                        <div className="text-sm text-gray-600 mb-1">Duration</div>
                        <div className="text-2xl font-bold text-purple-600">
                          {bandBaja.duration.minimum}-{bandBaja.duration.maximum || bandBaja.duration.minimum} hrs
                        </div>
                      </div>
                      <div className="bg-purple-50 p-4 rounded-lg">
                        <div className="text-sm text-gray-600 mb-1">Experience</div>
                        <div className="text-2xl font-bold text-purple-600">{bandBaja.experience} Years</div>
                      </div>
                    </div>
                    ...
                    <div className="mb-6">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        {bandBaja.features.map((feature, index) => (
                ...
                {activeTab === 'instruments' && (
                  <div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                      {bandBaja.instruments.map((instrument, index) => (
                ...
                {activeTab === 'performances' && (
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {bandBaja.performances.map((performance, index) => (
                        <div key={index} className="flex items-center bg-purple-50 p-4 rounded-lg">
                ...
                {activeTab === 'costumes' && (
                  <div>
                    <h2 className="text-2xl font-bold text-gray-800 mb-4">Band Costumes</h2>
                    {bandBaja.costumes.included ? (
                      <div>
                        ...
                        <h3 className="text-xl font-semibold text-gray-800 mb-3">Available Costume Types</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {bandBaja.costumes.types.map((type, index) => (
                            <div key={index} className="bg-purple-50 p-4 rounded-lg flex items-center">
                      ...
                      <div className="bg-yellow-100 text-yellow-800 px-4 py-3 rounded-lg">
                        Costumes are not included. Please contact for costume arrangements.
                      </div>
                    )}
                  </div>
                )}
                ...
                {activeTab === 'reviews' && (
                  <div>
                    ...
                    {bandBaja.reviews && bandBaja.reviews.length > 0 ? (
                      <div className="space-y-4">
                        {bandBaja.reviews.map((review, index) => (
            ...
            <div className="sticky top-8 space-y-6">
              ...
              <div className="bg-white rounded-lg shadow-lg p-6">
                ...
                <div className="mb-6">
                  ...
                  <div className="text-3xl font-bold text-purple-600">₹{(bandBaja.price.perEvent / 1000).toFixed(0)}k</div>
                  {bandBaja.price.perHour && (
                    <div className="text-sm text-gray-600 mt-2">₹{(bandBaja.price.perHour / 1000).toFixed(1)}k per hour</div>
                  )}
                </div>

                <div className="space-y-3 mb-6 border-t pt-4">
                  <div className="flex justify-between">
                    <span className="text-gray-600">Band Members</span>
                    <span className="font-semibold text-gray-800">{bandBaja.bandMembers}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Dhol Players</span>
                    <span className="font-semibold text-gray-800">{bandBaja.dholPlayers}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold text-gray-800">{bandBaja.lightingIncluded ? 'Included' : 'Not Included'}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Sound System</span>
                    <span className="font-semibold text-gray-800">{bandBaja.soundSystemIncluded ? 'Included' : 'Not Included'}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Location</span>
                    <span className="font-semibold text-gray-800">{bandBaja.location.city}</span>
                  </div>
                </div>
              ...
              <div className="bg-white rounded-lg shadow-lg p-6">
                ...
                <div className="space-y-3">
                  <a
                    href={`tel:${bandBaja.contactInfo.phone}`}
                  ...
                  >
                    ...
                    {bandBaja.contactInfo.phone}
                  </a>
                  <a
                    href={`mailto:${bandBaja.contactInfo.email}`}
                    className="flex items-center text-gray-700 hover:text-purple-600"
                  >
                    <svg className="w-5 h-5 mr-3 text-purple-600" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    {bandBaja.contactInfo.email}
                  </a>
                  {bandBaja.contactInfo.whatsapp && (
                    <a
                      href={`https://wa.me/${bandBaja.contactInfo.whatsapp}`}
                      target="_blank"
                  ...
                  {bandBaja.contactInfo.instagram && (
                    <a
                      href={`https://instagram.com/${bandBaja.contactInfo.instagram}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center text-gray-700 hover:text-purple-600"
                    >
...
export default BandBajaDetails;
