...
function BanquetHallDetails() {
  ...
  const fetchHallDetails = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`http://localhost:5000/api/banquet-halls/${id}`);
    ...
    return (
      <div className="min-h-screen flex justify-center items-center bg-gradient-to-br from-blue-50 to-indigo-100">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-gray-800 mb-4">Banquet Hall Not Found</h2>
          <button
            onClick={() => navigate('/venue-hall/banquet-hall')}
            ...
            {hall.images.map((image, index) => (
              <img
                ...
                className={`w-full h-24 object-cover rounded cursor-pointer transition-all ${
                  selectedImage === index ? 'ring-4 ring-indigo-600' : 'opacity-70 hover:opacity-100'
                }`}
              ...
              <div className="flex items-start justify-between mb-6">
                <div>
                  ...
                  <div className="flex items-center text-gray-600 mb-2">
                    ...
                    <span>{hall.location.area}, {hall.location.city} - {hall.location.pincode}</span>
                  </div>
              ...
              <div>
                {activeTab === 'about' && (
                  <div>
                    ...
                    <p className="text-gray-600 leading-relaxed mb-6">{hall.about}</p>
                    <div className="grid grid-cols-2 gap-4">
                      <div className="bg-indigo-50 p-4 rounded-lg">
                        <p className="text-sm text-gray-600 mb-1">Min Capacity</p>
                        <p className="text-2xl font-bold text-indigo-600">{hall.capacity.min} guests</p>
                      </div>
                      <div className="bg-indigo-50 p-4 rounded-lg">
                        <p className="text-sm text-gray-600 mb-1">Max Capacity</p>
                        <p className="text-2xl font-bold text-indigo-600">{hall.capacity.max} guests</p>
                      </div>
                    </div>
                  </div>
                )}
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-4 mt-4">
                        <div className="flex items-center space-x-2">
                          <svg className={`w-5 h-5 ${hall.policies.pets ? 'text-green-500' : 'text-red-500'}`} fill="currentColor" viewBox="0 0 20 20">
                            <path fillRule="evenodd" d={hall.policies.pets ? "M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" : "M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z"} clipRule="evenodd" />
                          </svg>
                          <span className="text-gray-700">{hall.policies.pets ? 'Pets Allowed' : 'No Pets'}</span>
                        </div>
                        <div className="flex items-center space-x-2">
                          <svg className={`w-5 h-5 ${hall.policies.smoking ? 'text-green-500' : 'text-red-500'}`} fill="currentColor" viewBox="0 0 20 20">
                            <path fillRule="evenodd" d={hall.policies.smoking ? "M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" : "M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z"} clipRule="evenodd" />
                          </svg>
                          <span className="text-gray-700">{hall.policies.smoking ? 'Smoking Allowed' : 'No Smoking'}</span>
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            <div className="bg-white rounded-lg shadow-lg p-6 sticky top-8">
              ...
              <div className="space-y-4 mb-6">
                <div className="flex items-center justify-between py-3 border-b">
                  ...
                  <span className="font-semibold">{hall.capacity.min} - {hall.capacity.max}</span>
                </div>
                <div className="flex items-center justify-between py-3 border-b">
                  <span className="text-gray-600">Type</span>
                  <span className="font-semibold">{hall.type}</span>
                </div>
                <div className="flex items-center justify-between py-3 border-b">
                  <span className="text-gray-600">Location</span>
                  <span className="font-semibold">{hall.location.city}</span>
                </div>
              </div>
              ...
              <button 
                onClick={() => {
                  if (!user) {
                    ...
                    navigate(`/guest/book/banquetHall/${id}`)
                  }
                }}
              ...
              <button className="w-full border-2 border-indigo-600 text-indigo-600 py-3 rounded-lg hover:bg-indigo-50 transition-colors font-semibold">
                Request Callback
              </button>
              ...
              <div className="mt-6 pt-6 border-t">
                ...
                <div className="space-y-2">
                  {hall.contactInfo.phone && (
                    <a href={`tel:${hall.contactInfo.phone}`} className="flex items-center text-gray-600 hover:text-indigo-600">
                      <svg className="w-5 h-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  ...
                  {hall.contactInfo.email && (
                    <a href={`mailto:${hall.contactInfo.email}`} className="flex items-center text-gray-600 hover:text-indigo-600">
                      <svg className="w-5 h-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  ...
                  {hall.contactInfo.website && (
                    <a href={`https://${hall.contactInfo.website}`} target="_blank" rel="noopener noreferrer" className="flex items-center text-gray-600 hover:text-indigo-600">
                      <svg className="w-5 h-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
