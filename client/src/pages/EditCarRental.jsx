...
const EditCarRental = () => {
  ...
  const [currentPackageCar, setCurrentPackageCar] = useState('');
  ...
  const fetchListing = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:5000/api/car-rentals/${id}`, {
  ...
  const addBrand = () => {
    if (currentBrand.trim()) {
      setFormData({
        ...formData,
        carBrands: [...formData.carBrands, currentBrand.trim()]
      });
      setCurrentBrand('');
    }
  };
  ...
  const removeBrand = (index) => {
    setFormData({
      ...formData,
      carBrands: formData.carBrands.filter((_, i) => i !== index)
    });
  };
  ...
  const addPackageCar = () => {
    if (currentPackageCar.trim()) {
      setCurrentPackage({
        ...currentPackage,
        cars: [...currentPackage.cars, currentPackageCar.trim()]
      });
      setCurrentPackageCar('');
    }
  };

  const removePackageCar = (index) => {
    setCurrentPackage({
      ...currentPackage,
      cars: currentPackage.cars.filter((_, i) => i !== index)
    });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        `http://localhost:5000/api/car-rentals/${id}`,
        formData,
        {
          headers: {
      ...
      if (response.data.success) {
        alert('Car Rental listing updated successfully!');
        navigate('/vendor/listings');
      }
    } catch (error) {
      console.error('Error updating car rental listing:', error);
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-4xl mx-auto px-4">
        <h1 className="text-3xl font-bold text-gray-900 mb-8">Edit Car Rental</h1>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-md p-6 space-y-6">
          <div>
            ...
            <input
              ...
              placeholder="Enter car rental name"
            />
          </div>
            ...
            <div className="bg-gray-50 p-4 rounded-lg mb-4 space-y-4">
              ...
              <div>
                ...
                <div className="flex gap-2 mb-2">
                  <input
                    type="text"
                    value={currentPackageCar}
                    onChange={(e) => setCurrentPackageCar(e.target.value)}
                    className="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                    placeholder="Add car"
                  />
                  <button
            ...
            <div className="space-y-3">
              {formData.packages.map((pkg, index) => (
                <div key={index} className="bg-white border border-gray-200 p-4 rounded-lg">
                  ...
                  {pkg.cars.length > 0 && (
                    <div className="mb-2">
                      <p className="text-xs font-semibold text-gray-700 mb-1">Cars:</p>
