...
function EditCommunityHall() {
  ...
  useEffect(() => {
    ...
    if (location.state?.listing) {
      ...
      fetchCommunityHallData();
    }
  }, [id, location.state]);

  const fetchCommunityHallData = async () => {
    try {
      ...
      console.error('Error fetching community hall data:', error);
      alert('Failed to load community hall data');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert('Community Hall updated successfully!');
      navigate('/vendor/listings');
    } catch (error) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 py-8">
      <div className="container mx-auto px-4 max-w-4xl">
        <div className="bg-white rounded-lg shadow-lg p-8">
          <h1 className="text-3xl font-bold text-gray-800 mb-6">Edit Community Hall</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-6">
                  <label className="flex items-center space-x-2">
                    <input
                      type="checkbox"
                      name="policies.religious"
                      checked={formData.policies.religious}
                      onChange={handleChange}
                      className="w-4 h-4 text-orange-600"
                    />
                    <span>Religious Events Allowed</span>
                  </label>
                  <label className="flex items-center space-x-2">
                    <input
                      type="checkbox"
                      name="policies.catering"
            ...
            <div className="flex gap-4 pt-6">
              <button
                type="submit"
                disabled={loading}
                className="flex-1 bg-gradient-to-r from-orange-600 to-red-600 text-white py-3 rounded-md font-semibold hover:from-orange-700 hover:to-red-700 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {loading ? 'Updating...' : 'Update Community Hall'}
              </button>
              <button
                type="button"
...
export default EditCommunityHall;
