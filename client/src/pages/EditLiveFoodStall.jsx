...
const EditLiveFoodStall = () => {
  ...
  const fetchListing = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:5000/api/live-food-stalls/${id}`, {
  ...
  const addDish = () => {
    if (currentDish.trim()) {
      setFormData({
        ...formData,
        dishVarieties: [...formData.dishVarieties, currentDish.trim()]
      });
      setCurrentDish('');
    }
  };
  ...
  const removeDish = (index) => {
    setFormData({
      ...formData,
      dishVarieties: formData.dishVarieties.filter((_, i) => i !== index)
    });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        `http://localhost:5000/api/live-food-stalls/${id}`,
        formData,
        {
          headers: {
      ...
      if (response.data.success) {
        alert('Live Food Stall listing updated successfully!');
        navigate('/vendor/listings');
      }
    } catch (error) {
      console.error('Error updating live food stall listing:', error);
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-4xl mx-auto px-4">
        <h1 className="text-3xl font-bold text-gray-900 mb-8">Edit Live Food Stall</h1>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-md p-6 space-y-6">
          <div>
            ...
            <input
              ...
              placeholder="Enter food stall name"
            />
          </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {formData.dishVarieties.map((dish, index) => (
                ...
                <div className="flex items-center">
                  <label className="flex items-center space-x-2">
                    <input
                      type="checkbox"
                      name="setupIncluded"
                      checked={currentPackage.setupIncluded}
                      onChange={handlePackageChange}
                      className="w-4 h-4 text-blue-600"
                    />
                    <span className="text-sm font-medium text-gray-700">Setup Included</span>
                  </label>
                </div>
              </div>
              <div>
            ...
            <div className="space-y-3">
              {formData.packages.map((pkg, index) => (
                <div key={index} className="bg-white border border-gray-200 p-4 rounded-lg">
                  ...
                  <p className="text-sm text-gray-600 mb-2">Servings: {pkg.servings}</p>
                  <p className="text-sm text-gray-600 mb-2">Setup: {pkg.setupIncluded ? 'Included' : 'Not Included'}</p>
                  {pkg.items.length > 0 && (
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <input
                type="number"
                name="dishesAvailable"
                value={formData.dishesAvailable}
                onChange={handleChange}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
          ...
          <div>
            ...
            <textarea
              ...
              rows="4"
              className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
              placeholder="Describe your live food stall service..."
            />
          </div>
          ...
          <div className="flex gap-4 pt-6">
            ...
            >
              Update Live Food Stall Listing
            </button>
            <button
              type="button"
...
export default EditLiveFoodStall;
