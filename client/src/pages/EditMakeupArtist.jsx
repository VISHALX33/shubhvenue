...
const EditMakeupArtist = () => {
  ...
  const fetchListing = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:5000/api/makeup-artists/${id}`, {
  ...
  const addStyle = () => {
    if (currentStyle.trim()) {
      setFormData({
        ...formData,
        makeupStyles: [...formData.makeupStyles, currentStyle.trim()]
      });
      setCurrentStyle('');
    }
  };
  ...
  const removeStyle = (index) => {
    setFormData({
      ...formData,
      makeupStyles: formData.makeupStyles.filter((_, i) => i !== index)
    });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        `http://localhost:5000/api/makeup-artists/${id}`,
        formData,
        {
          headers: {
      ...
      if (response.data.success) {
        alert('Makeup Artist listing updated successfully!');
        navigate('/vendor/listings');
      }
    } catch (error) {
      console.error('Error updating makeup artist listing:', error);
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-4xl mx-auto px-4">
        <h1 className="text-3xl font-bold text-gray-900 mb-8">Edit Makeup Artist</h1>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-md p-6 space-y-6">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {formData.makeupStyles.map((style, index) => (
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              ...
              <input
                type="number"
                name="bridalMakeupCount"
                value={formData.bridalMakeupCount}
                onChange={handleChange}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
          ...
          <div>
            ...
            <textarea
              ...
              rows="4"
              className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
              placeholder="Describe your makeup artist services..."
            />
          </div>
